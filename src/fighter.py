...
class Fighter:
    ...
    def can_act(self):
        """Kiểm tra xem nhân vật có đang rảnh để thực hiện hành động mới không."""
        # Nhân vật CÓ THỂ hành động nếu họ KHÔNG bị dính đòn, KHÔNG đang tấn công,
        # KHÔNG đã chết và KHÔNG đang lăn né.
    ...
    def attack(self, target, attack_type):
        # === LOGIC COMBO ĐÃ SỬA LỖI ===
        # Nếu đang trong animation của đòn light và người chơi nhấn light lần nữa...
        if self.attacking and 'light' in self.action and 'light' in attack_type:
            # ...thì ghi nhận lại thời điểm nhấn phím để thực hiện combo nối.
            self.combo_input_timer = pygame.time.get_ticks()
            return # Không làm gì thêm, chỉ "đệm" lệnh

        # Kiểm tra các điều kiện để có thể tấn công
        ...
        # Xử lý các loại tấn công khác
        if self.in_air:
            if attack_type == 'air' and 'air' in self.attacks: self.perform_attack('air', target)
            return
            
        if attack_type == 'special':
            if self.use_sp(SP_COST_SPECIAL): self.perform_attack('special', target)
        
        # Khi bắt đầu một chuỗi combo mới, reset về đòn 1
        elif attack_type == 'light':
            self.combo_step = 1
            self.perform_attack('light1', target)
            self.combo_input_timer = 0 # Xóa bộ đệm lệnh cũ

    def perform_attack(self, attack_type, target=None):
        if attack_type == 'special' and target is not None and self.character_type == 'B':
    ...
    def update(self):
        ...
        # Ưu tiên kiểm tra trạng thái chết trước nhất
        if self.dead:
            self.action = 'death'
        else:
            # --- LOGIC TIMERS ---
            ...
            if self.hit and pygame.time.get_ticks() - self.hit_stun_timer > self.hit_stun_duration:
                self.hit = False
            
            # --- LOGIC MỚI: XỬ LÝ KHI ROLL KẾT THÚC ---
            if self.rolling and pygame.time.get_ticks() - self.roll_timer > ROLL_DURATION:
                ...
                # Sau khi roll xong, nếu vẫn đang ở trên không, bắt đầu rơi xuống
                if self.in_air:
                    self.action = 'jump_down' # Gán trực tiếp trạng thái rơi
            
            if not self.attacking and not self.hit:
                if pygame.time.get_ticks() - self.passive_sp_timer > PASSIVE_SP_GAIN_RATE:
                    self.gain_sp(PASSIVE_SP_GAIN_AMOUNT)
                    self.passive_sp_timer = pygame.time.get_ticks()
            
            # Logic kết thúc tấn công & combo
            if self.attacking:
                if pygame.time.get_ticks() - self.attack_timer > self.attacks[self.action]['duration']:
                    ...
                    if 'light' in self.action and is_next_combo_input_valid:
                        ...
                        if next_attack in self.attacks:
                            self.perform_attack(next_attack, None)
    ...
    def update_animation(self):
        ...
        if pygame.time.get_ticks() - self.update_time > current_cooldown and not is_holding_frame:
            self.update_time = pygame.time.get_ticks()
            if not (self.action == 'death' and self.frame_index >= len(animation_frames) - 1):
                 self.frame_index += 1
    ...
    def move(self, target):
        ...
        # === LOGIC PHÒNG THỦ ĐÃ SỬA LỖI ===
        # Cho phép di chuyển lùi khi đang phòng thủ
        ...
        # Nếu đang phòng thủ, chỉ cho phép trượt lùi, không cho di chuyển tự do
        if self.defending and not is_sliding_back:
