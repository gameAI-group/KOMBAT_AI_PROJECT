...
class Fighter:
    def __init__(self, character_type, x, y, is_player_one):
        ...
        self.air_speed = self.stats['air_speed']
        self.attacks = self.stats['attacks']
        self.defense_modifier = self.stats['defense_modifier']
        self.sp_gain_on_hit = self.stats['sp_gain_on_hit']
        self.sp_gain_on_get_hit = self.stats['sp_gain_on_get_hit']
        self.start_x, self.start_y = x, y
        
        self.animation_speeds = self.stats.get('animation_speeds', {})
        self.animations = self.load_animations()
        
        # --- SỬA ĐỔI: Tải âm thanh trước khi gọi reset để có thể dùng ngay ---
        self.sfx = {}
        try:
            self.sfx['jump'] = pygame.mixer.Sound(SFX_JUMP)
            self.sfx['air_hit'] = pygame.mixer.Sound(SFX_AIR_HIT)
            # Sử dụng self.character_type thay vì char_key
            if self.character_type == 'A':
                self.sfx['hit'] = pygame.mixer.Sound(SFX_HIT_A)
            else:
                self.sfx['hit'] = pygame.mixer.Sound(SFX_HIT_B)
            
            for sound in self.sfx.values():
                sound.set_volume(0.6)
        except pygame.error as e:
            print(f"Cảnh báo: Không tải được SFX cho {self.name}: {e}")
    ...
    def attack(self, target, attack_type):
        ...
        if attack_type == 'special':
            if pygame.time.get_ticks() - self.last_special_time < SPECIAL_ATTACK_COOLDOWN: return 
    ...
    def create_and_check_hitbox(self, target, attack_data):
        ...
        if self.hitbox.colliderect(target.hurtbox):
            # --- MỚI: Xử lý âm thanh khi đòn đánh trúng ---
            # Nếu là đòn trên không, phát âm thanh 'air_hit'
            if self.action == 'air':
                if 'air_hit' in self.sfx: self.sfx['air_hit'].play()
            # Nếu là các đòn khác (light, special), phát âm thanh 'hit' mặc định
            else:
                if 'hit' in self.sfx: self.sfx['hit'].play()
            # -----------------------------------------------
    ...
    def take_hit(self, damage, knockback, stun_duration, attacker_is_flipped):
        ...
        else: self.hit = True; self.hit_stun_timer = pygame.time.get_ticks(); self.hit_stun_duration = stun_duration
    ...
    def roll(self):
        ...
        if can_normal_roll:
            ...
            if can_roll_on_platform and self.use_sp(SP_COST_ROLL):
                if self.in_air: self.air_rolls_left -= 1
        ...
        if can_tech_roll and (pygame.time.get_ticks() - self.hit_stun_timer < TECH_ROLL_WINDOW) and self.use_sp(SP_COST_TECH_ROLL):
            self.hit = False; self.tech_rolling = True; self.rolling = True
            self.roll_timer = pygame.time.get_ticks(); self.roll_velocity_x = ROLL_SPEED
    ...
    def jump(self):
        ...
        if can_jump:
            # --- MỚI: Phát âm thanh nhảy ---
            if 'jump' in self.sfx: self.sfx['jump'].play()
            # -------------------------------
            self.vel_y = JUMP_POWER; self.in_air = True; self.jumps_left -= 1

    def update(self):
        prev_action = self.action
        if self.dead: self.action = 'death'; self.invincible = False 
        else:
            ...
            if self.rolling:
                ...
                self.invincible = ROLL_IFRAME_START <= elapsed_roll_time < (ROLL_IFRAME_START + ROLL_IFRAME_DURATION)
                if elapsed_roll_time > ROLL_DURATION:
                    self.rolling = False; self.invincible = False; self.tech_rolling = False
                    self.roll_cooldown_timer = ROLL_COOLDOWN_DURATION; self.roll_velocity_x = 0
            else: self.invincible = False
            if not self.attacking and not self.hit and pygame.time.get_ticks() - self.passive_sp_timer > PASSIVE_SP_GAIN_RATE:
            ...
            if self.attacking:
                ...
                if pygame.time.get_ticks() - self.attack_timer > attack_duration:
                    ...
                    if 'light' in self.action and is_next_combo_input_valid:
                        self.combo_step += 1; next_attack = f'light{self.combo_step}'
                        if next_attack in self.attacks: self.perform_attack(next_attack, None); self.combo_input_timer = 0
        ...
        if self.action != prev_action: self.frame_index = 0; self.update_time = pygame.time.get_ticks()
    ...
    def move(self, target):
        ...
        if self.rolling: dx += self.roll_velocity_x * (1 if not self.flip else -1)
        ...
        if (self.attacking and self.in_air) or self.rolling: self.vel_y = 0
        ...
        if not self.tech_rolling and temp_rect.colliderect(target.rect): dx = 0
    ...
    def draw(self, surface):
        ...
        if self.invincible: flipped_image.set_alpha(150)
        else: flipped_image.set_alpha(255)
