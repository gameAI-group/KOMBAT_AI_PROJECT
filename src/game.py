...
class Game:
    ...
    def reset_game(self):
        ...
        print(f"Đã chọn độ khó: {self.difficulty_choice}")
        if self.difficulty_choice == "EASY":
            self.ai_controller = AIRandom(self.ai, self.player)
        elif self.difficulty_choice == "MEDIUM":
            self.ai_controller = AIRuleBased(self.ai, self.player)
        elif self.difficulty_choice == "HARD":
    # Khi bạn code xong AI Hard, bạn sẽ dùng nó ở đây.
    # Tạm thời, có thể dùng lại AI RuleBased hoặc AIRandom.
            self.ai_controller = AIRuleBased(self.ai, self.player)
