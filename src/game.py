...
def load_animation_frames(path):
    ...
    if not os.path.isdir(path):
        ...
        return []
    frames = []
    # Sắp xếp file theo tên để đảm bảo thứ tự frame đúng
...
class Game:
    def __init__(self):
        ...
        # --- PHẦN BỊ THIẾU GÂY LỖI ---
        # Khởi tạo các biến cho animation màn hình chọn nhân vật
        # Dòng này phải tồn tại TRƯỚC KHI gọi self.load_assets()
        self.select_anims = {}
    ...
    def load_assets(self):
        try:
            ...
            # Giả sử có một thư mục cho cursor động, ví dụ: 'assets/images/ui/cursor_anim/'
            # Nếu không có, nó sẽ dùng CURSOR_P1 làm dự phòng
            ...
            print("Vui lòng kiểm tra lại đường dẫn trong config.py và sự tồn tại của file/thư mục.")
            self.running = False
